...
export default function AdminSupport() {
  ...
  return <main className="min-h-screen bg-zinc-50 py-8"><div className="mx-auto max-w-6xl px-4"><h1 className="text-3xl font-bold">{t.support.merchantSupportTitle}</h1><div className="mt-6 rounded-3xl border bg-white p-5">{loading ? t.support.loading : rows.length === 0 ? <p className="text-sm text-zinc-500">{t.support.noConversations}</p> : <div className="space-y-2">{rows.map((r) => <Link href={`/admin/support/${r.id}`} key={r.id} className="flex justify-between gap-3 rounded-2xl border p-4 hover:bg-zinc-50"><div><b>{r.subject}</b><p className="mt-1 text-xs text-zinc-500">Store: {r.store_id}</p></div><span className="text-xs">{r.status}</span></Link>)}</div>}</div></div></main>;
}
