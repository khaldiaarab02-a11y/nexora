  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-zinc-50 p-6" dir="rtl">
      <div className="w-full max-w-lg rounded-[2rem] border border-zinc-200 bg-white p-8 text-center shadow-sm">
        ...
        <p className="mt-3 leading-7 text-zinc-500">شكرًا لك. تم تسجيل الطلب وسيتم التواصل معك لتأكيده.</p>
        {order && <div className="mt-6 rounded-2xl bg-zinc-50 p-5"><p className="text-sm text-zinc-500">رقم الطلب</p><p className="mt-1 text-2xl font-bold">#{order}</p></div>}
        {store && <Link href={`/shop/${store}`} className="mt-6 block rounded-xl bg-zinc-900 px-4 py-3 font-semibold text-white">العودة إلى المتجر</Link>}
      </div>
    </main>
  );
}