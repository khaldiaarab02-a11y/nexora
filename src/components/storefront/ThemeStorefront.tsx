    ...
    <main className="min-h-screen bg-[var(--nexora-page)] text-[var(--nexora-text)]" style={{
      ...
      <section className="mx-auto max-w-6xl px-5 pb-8 pt-10">
        <div className={theme.layout.hero}>
          <p className="text-sm font-medium opacity-60">{t.storefront.onlineStore}</p>
      ...
      <section className="mx-auto max-w-6xl px-5 pb-16">
        <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center">
          <input value={searchInput} onChange={(e) => onSearchChange(e.target.value)} placeholder={t.storefront.searchPlaceholder} className="w-full rounded-2xl border border-black/10 bg-white px-5 py-3.5 text-sm outline-none focus:border-[var(--nexora-accent)] sm:flex-1" />
        ...
        <div className="mb-6 flex items-end justify-between">
          <div><p className="text-sm opacity-50">{t.storefront.discoverCollection}</p><h2 className="mt-1 text-2xl font-bold">{t.storefront.productsTitle}</h2></div>
          <span className="text-sm opacity-50">{products.length} {t.storefront.productWord}</span>
        </div>

        {products.length === 0 ? (
          <div className="rounded-3xl border border-black/10 bg-white p-12 text-center">
            <h3 className="text-xl font-bold">{t.storefront.noProductsTitle}</h3>
            <p className="mt-2 text-sm opacity-60">{t.storefront.noProductsSubtitle}</p>
          </div>
        ) : (
              ...
              <article key={product.id} className={`group overflow-hidden transition hover:-translate-y-1 hover:shadow-xl ${theme.layout.card}`}>
                <Link href={`/shop/${store.slug}/product/${product.slug}`} className="block">
                  <div className="relative aspect-[4/3] overflow-hidden bg-black/5">
                    {product.image_url ? <img src={product.image_url} alt={product.name} className="h-full w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-sm opacity-40">{t.storefront.noImage}</div>}
                    <div className="absolute end-4 top-4 flex flex-col items-end gap-1.5">
                      {product.is_featured && <span className="rounded-full bg-white/95 px-3 py-1.5 text-xs font-semibold shadow-sm">{t.storefront.featuredBadge}</span>}
                      {product.extraImageCount > 0 && <span className="rounded-full bg-black/60 px-2.5 py-1 text-[10px] font-medium text-white">+{product.extraImageCount} {t.storefront.extraPhotosSuffix}</span>}
                    </div>
                  </div>
                  <div className="p-5 pb-2">
                    ...
                    <div className="mt-5 flex items-end justify-between gap-3">
                      ...
                      <span className={product.stock_quantity > 0 ? "text-sm text-emerald-600" : "text-sm text-red-500"}>{product.stock_quantity > 0 ? t.storefront.inStock : t.storefront.outOfStock}</span>
                    </div>
                  </div>
                </Link>
      ...
      <footer className={`${theme.layout.footer} py-8 text-center text-sm opacity-50`}>{t.storefront.footerPoweredByPrefix} {store.name} {t.storefront.footerPoweredBySuffix}</footer>
    </main>
  );
}
